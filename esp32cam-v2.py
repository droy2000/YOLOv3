...
def retrieve_frames(url, output_frames):
    ...
    while True:
        ...
        if a != -1 and b != -1:
            jpg = bytes[a:b+2]
            bytes = bytes[b+2:]
            frames = cv.imdecode(np.frombuffer(jpg, dtype=np.uint8), cv.IMREAD_COLOR)
...
while True:
    ...
    #-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        process_thread = threading.Thread(target=findObjects, args= (outputs, frame))
