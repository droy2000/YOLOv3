...
#----------------------------------------------STAGE-1------------------------------------------------------------------------------------------------
# Image input
cap = cv.VideoCapture('http://162.191.11.144:8000/-wvhttp-01-/GetOneShot?image_size=640x480&frame_count=1000000000')
#-----------------------------------------------------------------------------------------------------------------------------------------------------
...
# 'maximum supression" which will get the bounding box of maximum confidence number.
    ...
    for i in indices:
        ...
        cv.rectangle(frame, (x,y), (x+w, y+h), (22,255, 0),1)
        cv.putText(frame, f'{classNames[classIds[i]]}{int(confs[i]*100)}%', (max(0,x),max(30,y)), cv.FONT_HERSHEY_SIMPLEX, .65,(255,255,114),1)
...
# (number of channels), that have all be preprocessed in the same manner.
    blob = cv.dnn.blobFromImage(frame, 1/255,(int(img_size[1]), int(img_size[0])), [0,0,0],1, crop=False)
